...
def test_home_page(client):
    ...
    response = client.get('/')
    assert response.status_code == 200
    assert b'About' in response.data  # Verifica a existencia se 'About' está na página
...
def test_all_tasks_page_redirects_when_not_logged_in(client):
    ...
    assert response.status_code == 200
    assert b'Please log in' in response.data  # Verifica existencia de um login na página